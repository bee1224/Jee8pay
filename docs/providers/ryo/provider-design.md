...
> **RENAME NOTE（2026-08-20）**：本文由原 `CCAT Provider Design` 改名而來（`ccat` / `CCAT_IBON` → `ryo` / `RYO_IBON`）。`JAY` / `CHI` 兩個同平台上游的 adapter 是本文 RYO adapter 的逐字複製，只更換 `ifCode` / `wayCode` / params model 與訊息前綴；平台契約（Token / Collect / Query / APN / checksum）完全相同，見 [`contract-evidence.md`](contract-evidence.md)。

## Status

```text
JEE-C03 = PASS-WITH-DEBT
RYO_CONTRACT_PHASE = COMPLETE
RYO_RUNTIME_GATE = OPEN
RYO_RUNTIME_IMPLEMENTATION = COMPLETE
RYO_OFFLINE_ACCEPTANCE = PASS
RYO_DEVELOPMENT_DEPLOYMENT = PASS
RYO_LIVE_E2E = PASS
RYO_PRODUCTION_CANDIDATE = DEPLOYED / ACCEPTED
RYO_PRODUCTION_VALIDATION = NOT STARTED
V1_CUTOVER = NOT STARTED
NEXT = USER-SELECTED JEE-D01 OR LATER JEE-M01
```

Merchant-authenticated official specification `多元支付平台-WEBAPI介面規格(V1.28.1)` closes the C02 amount、status、APN identity and Create-idempotency blockers. P04 implements the design below through JeePay's native Provider extension points. JEE-E02 subsequently completed Development deployment and one real TWD 40 RYO ibon E2E. JEE-E04 deployed the same accepted artifact as an isolated Platform Production Candidate; credential/config binding、public callback activation and pilot transaction remain explicit later gates.
...
Non-goals：Refund、Transfer/Payout、Division、Channel User、Close Order、COCS、credit card、ATM、other RYO products、custom Provider UI、secret-management refactor、JeePay payment-domain redesign。

## Source of Truth

Evidence priority：merchant-authenticated RYO specification → official specification → official SDK → official implementation → observed behavior → prior V1 implementation → inference。
...
→ Spring bean ryoPaymentService
→ PaywayUtil
→ payway/RyoIbon
```

Upstream query remains `PayOrderReissue → ChannelOrderReissueService → ryoPayOrderQueryService`. Merchant `QueryOrderController` remains local-`PayOrder` only. APN is routed by `ChannelNoticeController` to `ryoChannelNoticeService`; JeePay Core owns state transition and merchant notification.
...
The authenticated specification removes the previous duplicate account fields：Token `username` and Collect `cust_id` are the same value. Phase-1 `RyoNormalMchParams` should therefore contain only:
...
| RYO | JeePay | Rule |
| --- | --- | --- |
| Token `username` / Collect `cust_id` / APN `api_id` | `RyoNormalMchParams.custId` | exact account equality |
...
`trans_id` is not returned by Query, so it never authorizes a transition alone. A first APN `trans_id` may be stored only after account/order/checksum/amount validation and authenticated Query confirms the same RYO order's state. Later callbacks must match stored `channelOrderNo`; a mismatch fails closed.

## Amount Mapping

JeePay `UnifiedOrderRQ.amount` and `PayOrder.amount` are `Long` minor units. RYO `order_amount` is whole TWD dollars and rejects decimals.

```text
require PayOrder.amount % 100 == 0
ryoOrderAmount = PayOrder.amount / 100
...
| JeePay source | RYO field | Conversion |
| --- | --- | --- |
...
| provider policy | `expire_date` | `yyyy-MM-dd` within RYO-configured range |
...
```text
RYO → ChannelNoticeController → ryoChannelNoticeService
...
3. Load that order's RYO config and require `api_id == custId`.
4. Verify the 32-hex checksum in constant time.
5. Query RYO with the same account and order key; validate Query `cust_order_no` and exact amount relationships.
...
8. Return RYO ACK through `ChannelRetMsg.responseEntity`; Core retains state transition and merchant-notify ownership.
...
- `RyoPaymentService`：generic provider dispatch only.
- `payway/RyoIbon`：validate ibon inputs、map Create、return waiting instructions; never mark issuance as paid.
- `RyoPayOrderQueryService`：authenticated Query、identity/amount validation and official state mapping; no direct DB transition.
- `RyoChannelNoticeService`：parse APN、verify account/checksum/order/amount/transaction、query authoritative state and build exact ACK; no replacement MQ/state machine.
- `RyoClient`：small shared Token/Collect HTTPS client、expiry and error normalization; TLS certificate verification mandatory.
- `RyoKit`：optional small pure checksum helper only; no general framework.

## SDK and HTTP Decision

Use direct HTTPS through a minimal `RyoClient`. Official SDKs are C# and PHP and remain protocol evidence, not Java dependencies. Reuse an existing JeePay/Spring/Hutool HTTP dependency; add no new client library and never disable TLS verification.
...
| Entity | Design |
| --- | --- |
| PayInterface | `ifCode=ryo`, normal merchant mode, ISV disabled, `configPageType=1`, `wayCodes=[{"wayCode":"RYO_IBON"}]` |
| PayWay | `wayCode=RYO_IBON`, display name `黑貓 PAY ibon 繳款` |
...
```text
jeepay/jeepay-payment/src/main/java/com/jeequan/jeepay/pay/channel/ryo/
...
jeepay/jeepay-core/src/main/java/com/jeequan/jeepay/core/model/params/ryo/
└── RyoNormalMchParams.java
```

Expected GREEN edits：`CS.IF_CODE.RYO`、`CS.PAY_WAY.RYO_IBON` and approved PayInterface/PayWay seed/config data. Explicitly do not modify `PayOrder`、`AbstractPayOrderController`、`ChannelNoticeController`、`PayOrderProcessService`、`PayMchNotifyService`、merchant notification MQ/retry、authentication or RBAC.
...
RYO_DEVELOPMENT_DEPLOYMENT = PASS
RYO_LIVE_E2E = PASS
RYO_PRODUCTION_CANDIDATE = DEPLOYED_AND_ACCEPTED
RYO_PRODUCTION_VALIDATION = NOT_STARTED
