...
DEVELOPMENT_PUBLIC_CALLBACK_BINDING = PASS (V2-ONLY HOST/PATH; RESTART-PERSISTENT EDGE)
PRODUCTION_RUNTIME_BINDING = JEE-E04 ISOLATED PRODUCTION CANDIDATE DEPLOYED; JEE-I05 ACCEPTED
```

The V2 Development runtime is bound to `server1.nnviopp.com` as isolated Compose project `jee8pay-v2-dev`; operational details are in [`../operations/ccat-v2-development.md`](../operations/ccat-v2-development.md). Its CCAT connectivity is explicitly `PRODUCTION` using a V2-only secure source; exactly one standalone Token authentication passed. The first controlled order attempt exposed TD-011: with `isys.cache-config=false`, the adapter read an empty context map instead of JeePay's native cache-aware Provider-param source. The deployed Provider-local resolver now uses `ConfigContextQueryService.queryNormalMchParams` for Create、Query and APN; cache-enabled/disabled and fail-closed regressions pass while `t_pay_interface_config.if_params` remains the single source of truth. A separately authorized new TWD 40 order then completed Create、human payment、validated APN、native SUCCESS、Merchant Notify and Query reconciliation. The V2-only public APN hostname routes only `/api/pay/notify/ccat`, and the V2 application origin derives that exact URL. JEE-N01 made the Sandbox edge network/config attachment declarative and restart-persistent; current lifecycle and External readiness gates are in [`../operations/sandbox-edge-recovery.md`](../operations/sandbox-edge-recovery.md).

JEE-E04 binds an isolated Platform Production Candidate to `server1.lp33ing.com` as Compose project `jee8pay-v2-production`; JEE-I05 independently accepts its source provenance、health、isolation、rollback and V1 non-interference. Operational details are in [`../operations/ccat-v2-production-candidate.md`](../operations/ccat-v2-production-candidate.md). Its DB、Redis、MQ、networks、volumes、loopback ports、configuration and secret store are V2-only. The exact I04 TD-011 artifact runs with both environment identities set to `PRODUCTION`; internal services and non-transactional CCAT DNS/TCP/TLS/time checks pass. Production CCAT credential intake、native Merchant/Application config and public callback activation remain explicit Human Gates. No Token、order、payment、transactional Query、APN expectation or general traffic cutover occurred.
