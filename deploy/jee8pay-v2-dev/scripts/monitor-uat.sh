...
echo "--- 4. 最近 15 分鐘 payment log 黑猫 PAY（RYO/JAY/CHI/JHD）事件數 ---"
sudo -n docker logs --since 15m "$PAYMENT" 2>&1 | grep -cE "RYO_CREATE|JAY_CREATE|CHI_CREATE|JHD_CREATE|RYO_APN|JAY_APN|CHI_APN|JHD_APN|RYO.*reject|JAY.*reject|CHI.*reject|JHD.*reject" || echo 0
