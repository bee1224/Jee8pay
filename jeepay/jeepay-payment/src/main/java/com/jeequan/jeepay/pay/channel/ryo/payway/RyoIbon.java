...
import com.jeequan.jeepay.pay.channel.ryo.RyoKit;
import com.jeequan.jeepay.pay.channel.ryo.RyoLogSanitizer;
import com.jeequan.jeepay.pay.channel.ryo.RyoMchParamsResolver;
import com.jeequan.jeepay.pay.channel.ryo.RyoPaymentService;
...
/** RYO ibon CVS 建单与 ambiguous-response recovery。 */
@Service
@Slf4j
public class RyoIbon extends RyoPaymentService {
    ...
    public RyoIbon(RyoClient client, RyoMchParamsResolver paramsResolver) {
        this.client = client;
    ...
    public AbstractRS pay(UnifiedOrderRQ bizRQ, PayOrder payOrder, MchAppConfigContext mchAppConfigContext) {
        RyoIbonOrderRS result = new RyoIbonOrderRS();
        Instant requestTimestamp = Instant.now();
        RyoNormalMchParams params = null;
        try {
            ...
            RyoException configuration = new RyoException(ErrorType.CONFIGURATION, e.getMessage());
    ...
    JSONObject buildAppendRequest(UnifiedOrderRQ bizRQ, PayOrder payOrder, RyoNormalMchParams params,
                                  String notifyUrl) throws RyoException {
    ...
    CreateResult executeCreate(RyoNormalMchParams params, PayOrder payOrder, JSONObject request)
            throws RyoException {
        try {
    ...
    private CreateResult recoverAfterAmbiguity(RyoNormalMchParams params, PayOrder payOrder,
                                               RyoException createFailure)
            throws RyoException {
        CollectResponse query;
        try {
            query = client.queryWithMetadata(params, payOrder.getPayOrderId());
        } catch (RyoException queryFailure) {
    ...
    private static void validateCreateResponse(JSONObject response, PayOrder payOrder, RyoNormalMchParams params,
        ...
        try {
            ...
            long responseAmount = RyoKit.wholeTwdToMinorUnits(response.get("order_amount"), "order_amount", false);
            if (responseAmount != payOrder.getAmount()) {
                throw new IllegalArgumentException("order_amount mismatch");
            }
            RyoKit.parseWholeTwd(response.get("bill_amount"), "bill_amount", false);
            ...
            throw new RyoException(ErrorType.MALFORMED, "RYO Create response validation failed", e,
                    metadata.getHttpStatus(), metadata.getLatencyMillis(), allowlistedProviderFields(response));
        }
    }

    private static void populateWaitingResult(RyoIbonOrderRS result, PayOrder payOrder, JSONObject response) {
        ...
        result.setBillAmount(RyoKit.parseWholeTwd(response.get("bill_amount"), "bill_amount", false));
    ...
    private static ChannelRetMsg errorResult(RyoException error) {
        if (error.getType() == ErrorType.BUSINESS) {
            return ChannelRetMsg.confirmFail("RYO_BUSINESS", "RYO Provider 拒絕請求");
        }
        ...
        result.setChannelErrCode("RYO_" + error.getType().name());
    ...
    private static JSONObject parsePayer(String channelExtra) {
        ...
        try {
            ...
            throw new IllegalArgumentException("RYO channelExtra 格式錯誤");
        }
        if (payer == null || StringUtils.isAnyBlank(
            ...
            throw new IllegalArgumentException("RYO channelExtra 缺少繳款人資料");
        }
        return payer;
    }
    ...
    private static String boundedOrderDetail(PayOrder payOrder) {
        ...
        if (StringUtils.isBlank(detail)) {
            throw new IllegalArgumentException("RYO order_detail 為必填");
        }
        return detail.length() <= ORDER_DETAIL_MAX_LENGTH ? detail : detail.substring(0, ORDER_DETAIL_MAX_LENGTH);
    }

    private static void requireProviderOk(CollectResponse response) throws RyoException {
    ...
    private static RyoException providerError(CollectResponse response) {
        String message = response.getBody() == null ? null : response.getBody().getString("msg");
        return new RyoException(ErrorType.BUSINESS,
                StringUtils.defaultIfBlank(message, "RYO Provider 拒絕請求"), null,
                response.getHttpStatus(), response.getLatencyMillis(), allowlistedProviderFields(response.getBody()));
    }

    private static boolean isAmbiguous(RyoException error) {
        return error.getType() == ErrorType.AMBIGUOUS || error.getType() == ErrorType.MALFORMED;
    }

    private static RyoException withReconciliation(RyoException error, String reconciliation) {
        ...
        return new RyoException(error.getType(), error.getMessage(), error.getCause(),
    ...
    private static void logCreate(PayOrder payOrder, Instant requestTimestamp, RyoNormalMchParams params,
                                  CollectResponse response, RyoException error, String outcome,
        ...
        log.info("event=RYO_CREATE operation=CREATE outcome={} payOrderId={} mchOrderNo={} "
                ...
                outcome, RyoLogSanitizer.sanitize(payOrder.getPayOrderId(), params),
                RyoLogSanitizer.sanitize(payOrder.getMchOrderNo(), params),
                requestTimestamp, RyoKit.toRyoTwdAmount(payOrder.getAmount()), value(httpStatus),
                RyoLogSanitizer.sanitize(value(fields, "status"), params),
                RyoLogSanitizer.sanitize(firstValue(fields, "process_code", "result_code", "code"), params),
                RyoLogSanitizer.sanitize(providerMessage, params),
                RyoLogSanitizer.sanitize(firstValue(fields, "trans_id", "transaction_id"), params),
                value(latencyMillis), reconciliation);
    }

    private static String outcome(RyoException error) {
