package com.jeequan.jeepay.pay.channel.chi.payway;
...
import com.jeequan.jeepay.pay.channel.chi.ChiClient;
import com.jeequan.jeepay.pay.channel.chi.ChiClient.ChiException;
import com.jeequan.jeepay.pay.channel.chi.ChiClient.CollectResponse;
import com.jeequan.jeepay.pay.channel.chi.ChiClient.ErrorType;
import com.jeequan.jeepay.pay.channel.chi.ChiIbonOrderRS;
import com.jeequan.jeepay.pay.channel.chi.ChiKit;
import com.jeequan.jeepay.pay.channel.chi.ChiLogSanitizer;
import com.jeequan.jeepay.pay.channel.chi.ChiMchParamsResolver;
import com.jeequan.jeepay.pay.channel.chi.ChiPaymentService;
...
/** CHI ibon CVS 建单与 ambiguous-response recovery。 */
@Service
@Slf4j
public class ChiIbon extends ChiPaymentService {
    ...
    private final ChiClient client;
    private final ChiMchParamsResolver paramsResolver;

    public ChiIbon(ChiClient client, ChiMchParamsResolver paramsResolver) {
        this.client = client;
    ...
    public AbstractRS pay(UnifiedOrderRQ bizRQ, PayOrder payOrder, MchAppConfigContext mchAppConfigContext) {
        ChiIbonOrderRS result = new ChiIbonOrderRS();
        Instant requestTimestamp = Instant.now();
        ChiNormalMchParams params = null;
        try {
            ...
            ChiException configuration = new ChiException(ErrorType.CONFIGURATION, e.getMessage());
    ...
    JSONObject buildAppendRequest(UnifiedOrderRQ bizRQ, PayOrder payOrder, ChiNormalMchParams params,
                                  String notifyUrl) throws ChiException {
        if (params == null || StringUtils.isAnyBlank(
                params.getEnvironment(), params.getCustId(), params.getApiPassword())) {
            throw new ChiException(ErrorType.CONFIGURATION, "CHI 商戶設定不完整");
        }
    ...
    CreateResult executeCreate(ChiNormalMchParams params, PayOrder payOrder, JSONObject request)
            throws ChiException {
        try {
    ...
    private CreateResult recoverAfterAmbiguity(ChiNormalMchParams params, PayOrder payOrder,
                                               ChiException createFailure)
            throws ChiException {
        CollectResponse query;
        try {
            query = client.queryWithMetadata(params, payOrder.getPayOrderId());
        } catch (ChiException queryFailure) {
    ...
    private static void validateCreateResponse(JSONObject response, PayOrder payOrder, ChiNormalMchParams params,
        ...
        try {
            ...
            long responseAmount = ChiKit.wholeTwdToMinorUnits(response.get("order_amount"), "order_amount", false);
            if (responseAmount != payOrder.getAmount()) {
                throw new IllegalArgumentException("order_amount mismatch");
            }
            ChiKit.parseWholeTwd(response.get("bill_amount"), "bill_amount", false);
            ...
            throw new ChiException(ErrorType.MALFORMED, "CHI Create response validation failed", e,
                    metadata.getHttpStatus(), metadata.getLatencyMillis(), allowlistedProviderFields(response));
        }
    }

    private static void populateWaitingResult(ChiIbonOrderRS result, PayOrder payOrder, JSONObject response) {
        ...
        result.setBillAmount(ChiKit.parseWholeTwd(response.get("bill_amount"), "bill_amount", false));
    ...
    private static ChannelRetMsg errorResult(ChiException error) {
        if (error.getType() == ErrorType.BUSINESS) {
            return ChannelRetMsg.confirmFail("CHI_BUSINESS", "CHI Provider 拒絕請求");
        }
        ...
        result.setChannelErrCode("CHI_" + error.getType().name());
    ...
    private static JSONObject parsePayer(String channelExtra) {
        ...
        try {
            ...
            throw new IllegalArgumentException("CHI channelExtra 格式錯誤");
        }
        if (payer == null || StringUtils.isAnyBlank(
            ...
            throw new IllegalArgumentException("CHI channelExtra 缺少繳款人資料");
        }
        return payer;
    }
    ...
    private static String boundedOrderDetail(PayOrder payOrder) {
        ...
        if (StringUtils.isBlank(detail)) {
            throw new IllegalArgumentException("CHI order_detail 為必填");
        }
        return detail.length() <= ORDER_DETAIL_MAX_LENGTH ? detail : detail.substring(0, ORDER_DETAIL_MAX_LENGTH);
    }

    private static void requireProviderOk(CollectResponse response) throws ChiException {
    ...
    private static ChiException providerError(CollectResponse response) {
        String message = response.getBody() == null ? null : response.getBody().getString("msg");
        return new ChiException(ErrorType.BUSINESS,
                StringUtils.defaultIfBlank(message, "CHI Provider 拒絕請求"), null,
                response.getHttpStatus(), response.getLatencyMillis(), allowlistedProviderFields(response.getBody()));
    }

    private static boolean isAmbiguous(ChiException error) {
        return error.getType() == ErrorType.AMBIGUOUS || error.getType() == ErrorType.MALFORMED;
    }

    private static ChiException withReconciliation(ChiException error, String reconciliation) {
        ...
        return new ChiException(error.getType(), error.getMessage(), error.getCause(),
    ...
    private static void logCreate(PayOrder payOrder, Instant requestTimestamp, ChiNormalMchParams params,
                                  CollectResponse response, ChiException error, String outcome,
        ...
        log.info("event=CHI_CREATE operation=CREATE outcome={} payOrderId={} mchOrderNo={} "
                ...
                outcome, ChiLogSanitizer.sanitize(payOrder.getPayOrderId(), params),
                ChiLogSanitizer.sanitize(payOrder.getMchOrderNo(), params),
                requestTimestamp, ChiKit.toChiTwdAmount(payOrder.getAmount()), value(httpStatus),
                ChiLogSanitizer.sanitize(value(fields, "status"), params),
                ChiLogSanitizer.sanitize(firstValue(fields, "process_code", "result_code", "code"), params),
                ChiLogSanitizer.sanitize(providerMessage, params),
                ChiLogSanitizer.sanitize(firstValue(fields, "trans_id", "transaction_id"), params),
                value(latencyMillis), reconciliation);
    }

    private static String outcome(ChiException error) {
