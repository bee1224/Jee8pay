...
import com.jeequan.jeepay.pay.channel.ryo.RyoPayOrderQueryService.ValidatedQuery;
...
/** RYO APN parser、identity binding 与 authenticated Query reconciliation。 */
@Slf4j
@Service
public class RyoChannelNoticeService extends AbstractChannelNoticeService {

    private static final String REQUEST_ORDER_ID = RyoChannelNoticeService.class.getName() + ".orderId";
    private static final String REQUEST_TRANS_ID = RyoChannelNoticeService.class.getName() + ".transId";

    private final RyoPayOrderQueryService queryService;
    private final PayOrderService payOrderService;
    private final RyoMchParamsResolver paramsResolver;

    public RyoChannelNoticeService(RyoPayOrderQueryService queryService, PayOrderService payOrderService,
                                    RyoMchParamsResolver paramsResolver) {
    ...
    public MutablePair<String, Object> parseParams(HttpServletRequest request, String urlOrderId,
        ...
        try {
            if (noticeTypeEnum != NoticeTypeEnum.DO_NOTIFY || !"POST".equals(request.getMethod())) {
                throw new IllegalArgumentException("RYO APN requires POST");
            }
            String contentType = request.getContentType();
            if (contentType == null || !contentType.toLowerCase().startsWith(MediaType.APPLICATION_JSON_VALUE)) {
                throw new IllegalArgumentException("RYO APN requires application/json");
            }
            String body = request.getReader().lines().collect(Collectors.joining());
            RyoNoticePayload payload = RyoNoticePayload.parse(body);
            ...
            log.warn("RYO APN parse rejected: {}", e.getMessage());
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder,
        ...
        try {
            if (noticeTypeEnum != NoticeTypeEnum.DO_NOTIFY || !(params instanceof RyoNoticePayload)) {
                throw new IllegalArgumentException("RYO APN payload is invalid");
            }
            RyoNoticePayload payload = (RyoNoticePayload) params;
            ...
            RyoNormalMchParams ryoParams = paramsResolver.resolve(mchAppConfigContext);
            if (!payload.apiId.equals(ryoParams.getCustId())) {
                throw new IllegalArgumentException("api_id mismatch");
            }
            if (!RyoKit.verifyChecksum(payload.apiId, payload.transId, payload.amount,
                    payload.status, payload.nonce, payload.checksum)) {
                throw new IllegalArgumentException("checksum mismatch");
            }

            ValidatedQuery query = queryService.queryValidated(payOrder, ryoParams);
            ...
            return result;
        } catch (RyoException | IllegalArgumentException e) {
            log.warn("RYO APN rejected for payOrderId={}: {}", payOrder.getPayOrderId(), e.getMessage());
            throw failureException();
        }
    }
    ...
    private static void validateApnAgainstQuery(RyoNoticePayload payload, ValidatedQuery query) {
        long apnAmount = RyoKit.parseWholeTwd(payload.amount, "amount", false);
        ...
        if ("B".equals(payload.status) || "E".equals(payload.status)) {
            ...
            long apnPayAmount = RyoKit.parseWholeTwd(payload.payAmount, "pay_amount", false);
    ...
    static final class RyoNoticePayload {
        ...
        private RyoNoticePayload(JSONObject body) {
            ...
            RyoKit.parseWholeTwd(amount, "amount", false);
            payDate = optional(body, "pay_date");
            payAmount = optional(body, "pay_amount");
        }

        static RyoNoticePayload parse(String rawBody) {
            if (StringUtils.isBlank(rawBody)) {
                throw new IllegalArgumentException("RYO APN body is empty");
            }
            try {
                ...
                return new RyoNoticePayload(body);
            } catch (JSONException e) {
                throw new IllegalArgumentException("RYO APN JSON is malformed");
            }
        }
        ...
        public String toString() {
            return "RyoNoticePayload{orderNo='" + orderNo + "', status='" + status + "'}";
        }
    }
}
