...
/** 最小 JHD Token / Collect HTTPS client。 */
@Component
public class JhdClient {
    ...
    JhdClient(Transport transport, Clock clock) {
        this.transport = transport;
        this.clock = clock;
    }

    public CollectResponse append(JhdNormalMchParams params, JSONObject payload) throws JhdException {
        return collect(params, payload);
    }

    public JSONObject query(JhdNormalMchParams params, String payOrderId) throws JhdException {
        return queryWithMetadata(params, payOrderId).getBody();
    }

    public CollectResponse queryWithMetadata(JhdNormalMchParams params, String payOrderId) throws JhdException {
    ...
    public static String resolveBaseUrl(String environment) throws JhdException {
        if (JhdNormalMchParams.ENVIRONMENT_TEST.equals(environment)) {
            return TEST_BASE_URL;
        }
        if (JhdNormalMchParams.ENVIRONMENT_PRODUCTION.equals(environment)) {
            return PRODUCTION_BASE_URL;
        }
        throw new JhdException(ErrorType.CONFIGURATION, "JHD 環境必須為 TEST 或 PRODUCTION");
    }

    private CollectResponse collect(JhdNormalMchParams params, JSONObject payload) throws JhdException {
        long startedAt = System.nanoTime();
        JhdNormalMchParams checked = requireConfig(params);
        ...
        try {
            ...
            throw new JhdException(ErrorType.AMBIGUOUS, "JHD Collect transport failure", e,
                    null, elapsedMillis(startedAt), null);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new JhdException(ErrorType.AMBIGUOUS, "JHD Collect interrupted", e,
                    null, elapsedMillis(startedAt), null);
        }
        ...
        if (response.statusCode == 401 || response.statusCode == 403) {
            throw new JhdException(ErrorType.AUTHENTICATION, "JHD Collect authentication failed", null,
        ...
        if (response.statusCode < 200 || response.statusCode >= 300) {
            ...
            throw new JhdException(type, "JHD Collect HTTP " + response.statusCode, null,
                    response.statusCode, latencyMillis, providerFields);
        }
        try {
            return new CollectResponse(
                    parseJson(response.body, ErrorType.MALFORMED, "JHD Collect response is malformed"),
                    response.statusCode, latencyMillis);
        } catch (JhdException e) {
            throw new JhdException(e.getType(), e.getMessage(), e,
                    response.statusCode, latencyMillis, providerFields);
        }
    }

    private String token(JhdNormalMchParams params, String baseUrl) throws JhdException {
        ...
        synchronized (lock) {
            ...
            try {
                ...
                throw new JhdException(ErrorType.AUTHENTICATION, "JHD Token transport failure", e);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new JhdException(ErrorType.AUTHENTICATION, "JHD Token interrupted", e);
            }

            if (response.statusCode < 200 || response.statusCode >= 300) {
                throw new JhdException(ErrorType.AUTHENTICATION, "JHD Token HTTP " + response.statusCode);
            }
            JSONObject body = parseJson(response.body, ErrorType.AUTHENTICATION, "JHD Token response is malformed");
            ...
            if (StringUtils.isAnyBlank(accessToken, expires) || StringUtils.isNotBlank(body.getString("error"))) {
                throw new JhdException(ErrorType.AUTHENTICATION, "JHD Token response rejected");
            }
            ...
            try {
                ...
                throw new JhdException(ErrorType.AUTHENTICATION, "JHD Token expiry is malformed", e);
            }
            Instant usableUntil = expiresAt.minus(TOKEN_EARLY_EXPIRY);
            if (!usableUntil.isAfter(now)) {
                throw new JhdException(ErrorType.AUTHENTICATION, "JHD Token is already expired");
            }
    ...
    private static JhdNormalMchParams requireConfig(JhdNormalMchParams params) throws JhdException {
    ...
    private static JSONObject parseJson(String body, ErrorType errorType, String message) throws JhdException {
        if (StringUtils.isBlank(body)) {
            throw new JhdException(errorType, message);
        }
        try {
            ...
            throw new JhdException(errorType, message, e);
        }
    }
    ...
    public static class JhdException extends Exception {
        private final ErrorType type;
        private final Integer httpStatus;
        private final Long latencyMillis;
        private final JSONObject providerFields;

        public JhdException(ErrorType type, String message) {
            this(type, message, null, null, null, null);
        }

        public JhdException(ErrorType type, String message, Throwable cause) {
            this(type, message, cause, null, null, null);
        }

        public JhdException(ErrorType type, String message, Throwable cause, Integer httpStatus,
