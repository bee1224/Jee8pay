...
/** CHI CvsOrderQuery adapter；仅回传 ChannelRetMsg，由 native reissue flow 转态。 */
@Service
public class ChiPayOrderQueryService implements IPayOrderQueryService {

    private final ChiClient client;
    private final ChiMchParamsResolver paramsResolver;

    public ChiPayOrderQueryService(ChiClient client, ChiMchParamsResolver paramsResolver) {
        this.client = client;
    ...
    public ChannelRetMsg query(PayOrder payOrder, MchAppConfigContext mchAppConfigContext) {
        try {
            ChiNormalMchParams params = paramsResolver.resolve(mchAppConfigContext);
    ...
    ValidatedQuery queryValidated(PayOrder payOrder, ChiNormalMchParams params) throws ChiException {
        JSONObject response = client.query(params, payOrder.getPayOrderId());
        if (!"OK".equals(response.getString("status"))) {
            String message = response.getString("msg");
            throw new ChiException(ErrorType.BUSINESS,
                    StringUtils.defaultIfBlank(message, "CHI Query 請求被拒絕"));
        }
        ...
        try {
            ...
            long localAmount = ChiKit.toChiTwdAmount(payOrder.getAmount());
            long orderAmount = ChiKit.parseWholeTwd(response.get("order_amount"), "order_amount", false);
            if (localAmount != orderAmount) {
                throw new IllegalArgumentException("order_amount mismatch");
            }
            long billAmount = ChiKit.parseWholeTwd(response.get("bill_amount"), "bill_amount", false);
            ...
            ChannelRetMsg.ChannelState state = ChiKit.mapProcessCode(processCode);

            Long payAmount = null;
            if (response.get("pay_amount") != null) {
                payAmount = ChiKit.parseWholeTwd(response.get("pay_amount"), "pay_amount", true);
            }
            ...
            throw new ChiException(ErrorType.MALFORMED, "CHI Query response validation failed", e);
        }
    }
    ...
    static final class ValidatedQuery {
        ...
        ChannelRetMsg toChannelRetMsg(String existingChannelOrderNo) {
            ...
            if (state == ChannelRetMsg.ChannelState.CONFIRM_SUCCESS) {
                ...
                result = ChannelRetMsg.unknown("CHI process_code 無安全轉態");
            }
