...
/** 最小 CHI Token / Collect HTTPS client。 */
@Component
public class ChiClient {
    ...
    ChiClient(Transport transport, Clock clock) {
        this.transport = transport;
        this.clock = clock;
    }

    public CollectResponse append(ChiNormalMchParams params, JSONObject payload) throws ChiException {
        return collect(params, payload);
    }

    public JSONObject query(ChiNormalMchParams params, String payOrderId) throws ChiException {
        return queryWithMetadata(params, payOrderId).getBody();
    }

    public CollectResponse queryWithMetadata(ChiNormalMchParams params, String payOrderId) throws ChiException {
    ...
    public static String resolveBaseUrl(String environment) throws ChiException {
        if (ChiNormalMchParams.ENVIRONMENT_TEST.equals(environment)) {
            return TEST_BASE_URL;
        }
        if (ChiNormalMchParams.ENVIRONMENT_PRODUCTION.equals(environment)) {
            return PRODUCTION_BASE_URL;
        }
        throw new ChiException(ErrorType.CONFIGURATION, "CHI 環境必須為 TEST 或 PRODUCTION");
    }

    private CollectResponse collect(ChiNormalMchParams params, JSONObject payload) throws ChiException {
        long startedAt = System.nanoTime();
        ChiNormalMchParams checked = requireConfig(params);
        ...
        try {
            ...
            throw new ChiException(ErrorType.AMBIGUOUS, "CHI Collect transport failure", e,
                    null, elapsedMillis(startedAt), null);
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new ChiException(ErrorType.AMBIGUOUS, "CHI Collect interrupted", e,
                    null, elapsedMillis(startedAt), null);
        }
        ...
        if (response.statusCode == 401 || response.statusCode == 403) {
            throw new ChiException(ErrorType.AUTHENTICATION, "CHI Collect authentication failed", null,
        ...
        if (response.statusCode < 200 || response.statusCode >= 300) {
            ...
            throw new ChiException(type, "CHI Collect HTTP " + response.statusCode, null,
                    response.statusCode, latencyMillis, providerFields);
        }
        try {
            return new CollectResponse(
                    parseJson(response.body, ErrorType.MALFORMED, "CHI Collect response is malformed"),
                    response.statusCode, latencyMillis);
        } catch (ChiException e) {
            throw new ChiException(e.getType(), e.getMessage(), e,
                    response.statusCode, latencyMillis, providerFields);
        }
    }

    private String token(ChiNormalMchParams params, String baseUrl) throws ChiException {
        ...
        synchronized (lock) {
            ...
            try {
                ...
                throw new ChiException(ErrorType.AUTHENTICATION, "CHI Token transport failure", e);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new ChiException(ErrorType.AUTHENTICATION, "CHI Token interrupted", e);
            }

            if (response.statusCode < 200 || response.statusCode >= 300) {
                throw new ChiException(ErrorType.AUTHENTICATION, "CHI Token HTTP " + response.statusCode);
            }
            JSONObject body = parseJson(response.body, ErrorType.AUTHENTICATION, "CHI Token response is malformed");
            ...
            if (StringUtils.isAnyBlank(accessToken, expires) || StringUtils.isNotBlank(body.getString("error"))) {
                throw new ChiException(ErrorType.AUTHENTICATION, "CHI Token response rejected");
            }
            ...
            try {
                ...
                throw new ChiException(ErrorType.AUTHENTICATION, "CHI Token expiry is malformed", e);
            }
            Instant usableUntil = expiresAt.minus(TOKEN_EARLY_EXPIRY);
            if (!usableUntil.isAfter(now)) {
                throw new ChiException(ErrorType.AUTHENTICATION, "CHI Token is already expired");
            }
    ...
    private static ChiNormalMchParams requireConfig(ChiNormalMchParams params) throws ChiException {
    ...
    private static JSONObject parseJson(String body, ErrorType errorType, String message) throws ChiException {
        if (StringUtils.isBlank(body)) {
            throw new ChiException(errorType, message);
        }
        try {
            ...
            throw new ChiException(errorType, message, e);
        }
    }
    ...
    public static class ChiException extends Exception {
        private final ErrorType type;
        private final Integer httpStatus;
        private final Long latencyMillis;
        private final JSONObject providerFields;

        public ChiException(ErrorType type, String message) {
            this(type, message, null, null, null, null);
        }

        public ChiException(ErrorType type, String message, Throwable cause) {
            this(type, message, cause, null, null, null);
        }

        public ChiException(ErrorType type, String message, Throwable cause, Integer httpStatus,
