package com.jeequan.jeepay.pay.channel.jhd.payway;
...
import com.jeequan.jeepay.pay.channel.jhd.JhdClient;
import com.jeequan.jeepay.pay.channel.jhd.JhdClient.JhdException;
import com.jeequan.jeepay.pay.channel.jhd.JhdClient.CollectResponse;
import com.jeequan.jeepay.pay.channel.jhd.JhdClient.ErrorType;
import com.jeequan.jeepay.pay.channel.jhd.JhdIbonOrderRS;
import com.jeequan.jeepay.pay.channel.jhd.JhdKit;
import com.jeequan.jeepay.pay.channel.jhd.JhdLogSanitizer;
import com.jeequan.jeepay.pay.channel.jhd.JhdMchParamsResolver;
import com.jeequan.jeepay.pay.channel.jhd.JhdPaymentService;
...
/** JHD ibon CVS 建单与 ambiguous-response recovery。 */
@Service
@Slf4j
public class JhdIbon extends JhdPaymentService {
    ...
    public JhdIbon(JhdClient client, JhdMchParamsResolver paramsResolver) {
        this.client = client;
    ...
    public AbstractRS pay(UnifiedOrderRQ bizRQ, PayOrder payOrder, MchAppConfigContext mchAppConfigContext) {
        JhdIbonOrderRS result = new JhdIbonOrderRS();
        Instant requestTimestamp = Instant.now();
        JhdNormalMchParams params = null;
        try {
            ...
            JhdException configuration = new JhdException(ErrorType.CONFIGURATION, e.getMessage());
    ...
    JSONObject buildAppendRequest(UnifiedOrderRQ bizRQ, PayOrder payOrder, JhdNormalMchParams params,
                                  String notifyUrl) throws JhdException {
        if (params == null || StringUtils.isAnyBlank(
                params.getEnvironment(), params.getCustId(), params.getApiPassword())) {
            throw new JhdException(ErrorType.CONFIGURATION, "JHD 商戶設定不完整");
        }
    ...
    CreateResult executeCreate(JhdNormalMchParams params, PayOrder payOrder, JSONObject request)
            throws JhdException {
        try {
    ...
    private CreateResult recoverAfterAmbiguity(JhdNormalMchParams params, PayOrder payOrder,
                                               JhdException createFailure)
            throws JhdException {
        CollectResponse query;
        try {
            query = client.queryWithMetadata(params, payOrder.getPayOrderId());
        } catch (JhdException queryFailure) {
    ...
    private static void validateCreateResponse(JSONObject response, PayOrder payOrder, JhdNormalMchParams params,
        ...
        try {
            ...
            long responseAmount = JhdKit.wholeTwdToMinorUnits(response.get("order_amount"), "order_amount", false);
            if (responseAmount != payOrder.getAmount()) {
                throw new IllegalArgumentException("order_amount mismatch");
            }
            JhdKit.parseWholeTwd(response.get("bill_amount"), "bill_amount", false);
            ...
            throw new JhdException(ErrorType.MALFORMED, "JHD Create response validation failed", e,
                    metadata.getHttpStatus(), metadata.getLatencyMillis(), allowlistedProviderFields(response));
        }
    }

    private static void populateWaitingResult(JhdIbonOrderRS result, PayOrder payOrder, JSONObject response) {
        ...
        result.setBillAmount(JhdKit.parseWholeTwd(response.get("bill_amount"), "bill_amount", false));
    ...
    private static ChannelRetMsg errorResult(JhdException error) {
        if (error.getType() == ErrorType.BUSINESS) {
            return ChannelRetMsg.confirmFail("JHD_BUSINESS", "JHD Provider 拒絕請求");
        }
        ...
        result.setChannelErrCode("JHD_" + error.getType().name());
    ...
    private static JSONObject parsePayer(String channelExtra) {
        ...
        try {
            ...
            throw new IllegalArgumentException("JHD channelExtra 格式錯誤");
        }
        if (payer == null || StringUtils.isAnyBlank(
            ...
            throw new IllegalArgumentException("JHD channelExtra 缺少繳款人資料");
        }
        return payer;
    }
    ...
    private static String boundedOrderDetail(PayOrder payOrder) {
        ...
        if (StringUtils.isBlank(detail)) {
            throw new IllegalArgumentException("JHD order_detail 為必填");
        }
        return detail.length() <= ORDER_DETAIL_MAX_LENGTH ? detail : detail.substring(0, ORDER_DETAIL_MAX_LENGTH);
    }

    private static void requireProviderOk(CollectResponse response) throws JhdException {
    ...
    private static JhdException providerError(CollectResponse response) {
        String message = response.getBody() == null ? null : response.getBody().getString("msg");
        return new JhdException(ErrorType.BUSINESS,
                StringUtils.defaultIfBlank(message, "JHD Provider 拒絕請求"), null,
                response.getHttpStatus(), response.getLatencyMillis(), allowlistedProviderFields(response.getBody()));
    }

    private static boolean isAmbiguous(JhdException error) {
        return error.getType() == ErrorType.AMBIGUOUS || error.getType() == ErrorType.MALFORMED;
    }

    private static JhdException withReconciliation(JhdException error, String reconciliation) {
        ...
        return new JhdException(error.getType(), error.getMessage(), error.getCause(),
    ...
    private static void logCreate(PayOrder payOrder, Instant requestTimestamp, JhdNormalMchParams params,
                                  CollectResponse response, JhdException error, String outcome,
        ...
        log.info("event=JHD_CREATE operation=CREATE outcome={} payOrderId={} mchOrderNo={} "
                ...
                outcome, JhdLogSanitizer.sanitize(payOrder.getPayOrderId(), params),
                JhdLogSanitizer.sanitize(payOrder.getMchOrderNo(), params),
                requestTimestamp, JhdKit.toJhdTwdAmount(payOrder.getAmount()), value(httpStatus),
                JhdLogSanitizer.sanitize(value(fields, "status"), params),
                JhdLogSanitizer.sanitize(firstValue(fields, "process_code", "result_code", "code"), params),
                JhdLogSanitizer.sanitize(providerMessage, params),
                JhdLogSanitizer.sanitize(firstValue(fields, "trans_id", "transaction_id"), params),
                value(latencyMillis), reconciliation);
    }

    private static String outcome(JhdException error) {
