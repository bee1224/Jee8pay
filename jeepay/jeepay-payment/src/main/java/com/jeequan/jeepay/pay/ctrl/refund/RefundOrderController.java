...
public class RefundOrderController extends ApiController {
    ...
    public ApiRes refundOrder(){
        ...
        try {
            ...
            if(payOrder == null){
                throw new BizException("退款訂單不存在");
            }

            if(payOrder.getState() != PayOrder.STATE_SUCCESS){
                throw new BizException("訂單狀態不正確，無法完成退款");
            }
            ...
            if(payOrder.getRefundAmount() + rq.getRefundAmount() > payOrder.getAmount()){
                throw new BizException("申請金額超出訂單可退款餘額，請檢查退款金額");
            }
            ...
            if(refundOrderService.count(RefundOrder.gw().eq(RefundOrder::getPayOrderId, payOrder.getPayOrderId()).eq(RefundOrder::getState, RefundOrder.STATE_ING)) > 0){
                throw new BizException("支付訂單具有在途退款申請，請稍後再試");
            }
            ...
            if(refundOrderService.count(RefundOrder.gw().eq(RefundOrder::getMchNo, mchNo).eq(RefundOrder::getMchRefundNo, rq.getMchRefundNo())) > 0){
                throw new BizException("商戶退款訂單號["+rq.getMchRefundNo()+"]已存在");
            }
            ...
            if(refundService == null){
                throw new BizException("當前通道不支援退款！");
            }

            refundOrder = genRefundOrder(rq, payOrder, mchInfo, mchApp);

            //退款单入库 退款单狀態：生成狀態  此时没有和任何上游渠道产生交互。
    ...
    private RefundOrder genRefundOrder(RefundOrderRQ rq, PayOrder payOrder, MchInfo mchInfo, MchApp mchApp){
        ...
        refundOrder.setRefundOrderId(SeqKit.genRefundOrderId()); //退款訂單号
        refundOrder.setPayOrderId(payOrder.getPayOrderId()); //支付訂單号
        refundOrder.setChannelPayOrderNo(payOrder.getChannelOrderNo()); //渠道支付单号
        refundOrder.setMchNo(mchInfo.getMchNo()); //商戶號
        refundOrder.setIsvNo(mchInfo.getIsvNo()); //服務商号
        refundOrder.setAppId(mchApp.getAppId()); //商戶應用ID
        refundOrder.setMchName(mchInfo.getMchShortName()); //商戶名称
        refundOrder.setMchType(mchInfo.getType()); //商戶类型
        refundOrder.setMchRefundNo(rq.getMchRefundNo()); //商戶退款單號
        refundOrder.setWayCode(payOrder.getWayCode()); //支付方式代码
        refundOrder.setIfCode(payOrder.getIfCode()); //支付介面代码
        refundOrder.setPayAmount(payOrder.getAmount()); //支付金額,单位分
        refundOrder.setRefundAmount(rq.getRefundAmount()); //退款金額,单位分
        refundOrder.setCurrency(rq.getCurrency()); // ISO 4217 三位貨幣代碼
        refundOrder.setState(RefundOrder.STATE_INIT); //退款狀態:0-訂單生成,1-退款中,2-退款成功,3-退款失敗
        ...
        refundOrder.setChannelOrderNo(null); //渠道訂單号
        refundOrder.setErrCode(null); //渠道錯誤码
        refundOrder.setErrMsg(null); //渠道錯誤描述
        refundOrder.setChannelExtra(rq.getChannelExtra()); //特定渠道发起时额外參數
        refundOrder.setNotifyUrl(rq.getNotifyUrl()); //通知地址
        refundOrder.setExtParam(rq.getExtParam()); //扩展參數
        refundOrder.setExpiredTime(DateUtil.offsetHour(nowTime, 2)); //訂單超时關閉时间 默认两个小时
        refundOrder.setSuccessTime(null); //訂單退款成功时间
    ...
    private void updateInitOrderStateThrowException(byte orderState, RefundOrder refundOrder, ChannelRetMsg channelRetMsg){
        ...
        if(!isSuccess){
            throw new BizException("更新退款單異常！");
        }
