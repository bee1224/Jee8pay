...
import com.jeequan.jeepay.pay.channel.jhd.JhdPayOrderQueryService.ValidatedQuery;
...
/** JHD APN parser、identity binding 与 authenticated Query reconciliation。 */
@Slf4j
@Service
public class JhdChannelNoticeService extends AbstractChannelNoticeService {

    private static final String REQUEST_ORDER_ID = JhdChannelNoticeService.class.getName() + ".orderId";
    private static final String REQUEST_TRANS_ID = JhdChannelNoticeService.class.getName() + ".transId";

    private final JhdPayOrderQueryService queryService;
    private final PayOrderService payOrderService;
    private final JhdMchParamsResolver paramsResolver;

    public JhdChannelNoticeService(JhdPayOrderQueryService queryService, PayOrderService payOrderService,
                                    JhdMchParamsResolver paramsResolver) {
    ...
    public MutablePair<String, Object> parseParams(HttpServletRequest request, String urlOrderId,
        ...
        try {
            if (noticeTypeEnum != NoticeTypeEnum.DO_NOTIFY || !"POST".equals(request.getMethod())) {
                throw new IllegalArgumentException("JHD APN requires POST");
            }
            String contentType = request.getContentType();
            if (contentType == null || !contentType.toLowerCase().startsWith(MediaType.APPLICATION_JSON_VALUE)) {
                throw new IllegalArgumentException("JHD APN requires application/json");
            }
            String body = request.getReader().lines().collect(Collectors.joining());
            JhdNoticePayload payload = JhdNoticePayload.parse(body);
            ...
            log.warn("JHD APN parse rejected: {}", e.getMessage());
    ...
    public ChannelRetMsg doNotice(HttpServletRequest request, Object params, PayOrder payOrder,
        ...
        try {
            if (noticeTypeEnum != NoticeTypeEnum.DO_NOTIFY || !(params instanceof JhdNoticePayload)) {
                throw new IllegalArgumentException("JHD APN payload is invalid");
            }
            JhdNoticePayload payload = (JhdNoticePayload) params;
            ...
            JhdNormalMchParams jhdParams = paramsResolver.resolve(mchAppConfigContext);
            if (!payload.apiId.equals(jhdParams.getCustId())) {
                throw new IllegalArgumentException("api_id mismatch");
            }
            if (!JhdKit.verifyChecksum(payload.apiId, payload.transId, payload.amount,
                    payload.status, payload.nonce, payload.checksum)) {
                throw new IllegalArgumentException("checksum mismatch");
            }

            ValidatedQuery query = queryService.queryValidated(payOrder, jhdParams);
            ...
            return result;
        } catch (JhdException | IllegalArgumentException e) {
            log.warn("JHD APN rejected for payOrderId={}: {}", payOrder.getPayOrderId(), e.getMessage());
            throw failureException();
        }
    }
    ...
    private static void validateApnAgainstQuery(JhdNoticePayload payload, ValidatedQuery query) {
        long apnAmount = JhdKit.parseWholeTwd(payload.amount, "amount", false);
        ...
        if ("B".equals(payload.status) || "E".equals(payload.status)) {
            ...
            long apnPayAmount = JhdKit.parseWholeTwd(payload.payAmount, "pay_amount", false);
    ...
    static final class JhdNoticePayload {
        ...
        private JhdNoticePayload(JSONObject body) {
            ...
            JhdKit.parseWholeTwd(amount, "amount", false);
            payDate = optional(body, "pay_date");
            payAmount = optional(body, "pay_amount");
        }

        static JhdNoticePayload parse(String rawBody) {
            if (StringUtils.isBlank(rawBody)) {
                throw new IllegalArgumentException("JHD APN body is empty");
            }
            try {
                ...
                return new JhdNoticePayload(body);
            } catch (JSONException e) {
                throw new IllegalArgumentException("JHD APN JSON is malformed");
            }
        }
        ...
        public String toString() {
            return "JhdNoticePayload{orderNo='" + orderNo + "', status='" + status + "'}";
        }
    }
}
