...
/** CCAT CvsOrderQuery adapter；仅回传 ChannelRetMsg，由 native reissue flow 转态。 */
@Service
public class CcatPayOrderQueryService implements IPayOrderQueryService {
    ...
    ValidatedQuery queryValidated(PayOrder payOrder, CcatNormalMchParams params) throws CcatException {
        ...
        try {
            ...
            long localAmount = CcatKit.toCcatTwdAmount(payOrder.getAmount());
            long orderAmount = CcatKit.parseWholeTwd(response.get("order_amount"), "order_amount", false);
            ...
            if (response.get("pay_amount") != null) {
                payAmount = CcatKit.parseWholeTwd(response.get("pay_amount"), "pay_amount", true);
            }
            ...
            throw new CcatException(ErrorType.MALFORMED, "CCAT Query response validation failed", e);
        }
    }
