...
class CcatMchParamsResolverTest {
    ...
    private CcatMchParamsResolver resolver;

    @BeforeEach
    void setUp() {
        TableInfoHelper.initTableInfo(
                new MapperBuilderAssistant(new Configuration(), "ccat-test"), PayInterfaceConfig.class);
        ...
        resolver = new CcatMchParamsResolver(nativeQueryService);
    }
    ...
    @Test
    void cacheEnabledLoadsCcatParamsFromNativeContext() throws Exception {
        ...
        cached.getNormalMchParamsMap().put(CS.IF_CODE.CCAT, validParams());
    ...
    @Test
    void cacheDisabledLoadsCcatParamsFromCanonicalDatabaseRow() throws Exception {
        SysConfigService.IS_USE_CACHE = false;
        when(dbService.getOne(any())).thenReturn(configRow(validJson()));

        CcatNormalMchParams loaded = resolver.resolve(binding("M-TEST", "APP-TEST"));
    ...
    void missingProviderParamsFailClosed() {
        ...
        assertThrows(CcatException.class, () -> resolver.resolve(binding("M-TEST", "APP-TEST")));
    }
    ...
    void wrongMerchantBindingCannotReuseAnotherAppsParams() {
        ...
        expected.getNormalMchParamsMap().put(CS.IF_CODE.CCAT, validParams());
        when(cacheService.getMchAppConfigContext("M-EXPECTED", "APP-EXPECTED")).thenReturn(expected);

        assertThrows(CcatException.class,
    ...
    private static PayInterfaceConfig configRow(String json) {
        return new PayInterfaceConfig()
                ...
                .setInfoId("APP-TEST")
                .setIfCode(CS.IF_CODE.CCAT)
                .setIfParams(json)
                .setState(CS.YES);
    }

    private static CcatNormalMchParams validParams() {
