...
class JhdIbonTest {
    ...
    private JhdClient client;
    private JhdMchParamsResolver paramsResolver;
    private JhdIbon ibon;
    private JhdNormalMchParams params;
    private PayOrder payOrder;
    private UnifiedOrderRQ request;

    @BeforeEach
    void setUp() throws Exception {
        client = mock(JhdClient.class);
        paramsResolver = mock(JhdMchParamsResolver.class);
        ibon = new TestJhdIbon(client, paramsResolver);
        params = new JhdNormalMchParams();
    ...
    void providerCreateSuccessProducesWaitingInstructions() throws Exception {
        ...
        context.getNormalMchParamsMap().put(CS.IF_CODE.JHD, params);

        JhdIbonOrderRS result = (JhdIbonOrderRS) ibon.pay(request, payOrder, context);
    ...
    void deterministicBusinessErrorDoesNotQueryOrRetry() throws Exception {
        ...
        JhdException error = assertThrows(JhdException.class,
    ...
    void deterministicBusinessErrorUsesNativeConfirmFailAndCannotBuildEmptyPayData() throws Exception {
        ...
        assertEquals("JHD_BUSINESS", result.getChannelRetMsg().getChannelErrCode());
    ...
    void authenticationErrorDoesNotQueryOrRetry() throws Exception {
        when(client.append(eq(params), any(JSONObject.class))).thenThrow(
                new JhdException(JhdClient.ErrorType.AUTHENTICATION, "token rejected"));

        JhdException error = assertThrows(JhdException.class,
                () -> ibon.executeCreate(params, payOrder, new JSONObject()));

        assertEquals(JhdClient.ErrorType.AUTHENTICATION, error.getType());
        verify(client, never()).queryWithMetadata(any(), anyString());
        assertThrows(IllegalStateException.class, new JhdIbonOrderRS()::buildPayData);
    }

    @Test
    void timeoutThenQueryFoundRecoversWithoutSecondAppend() throws Exception {
        when(client.append(eq(params), any(JSONObject.class))).thenThrow(
                new JhdException(JhdClient.ErrorType.AMBIGUOUS, "read timeout"));
        when(client.queryWithMetadata(params, payOrder.getPayOrderId()))
                .thenReturn(response(successResponse(100)));

        JhdIbon.CreateResult recovered = ibon.executeCreate(params, payOrder, new JSONObject());
    ...
    void timeoutThenExactNotFoundDoesNotRetryAppend() throws Exception {
        ...
        when(client.append(params, sameRequest)).thenThrow(
                new JhdException(JhdClient.ErrorType.AMBIGUOUS, "read timeout"));
        when(client.queryWithMetadata(params, payOrder.getPayOrderId()))
                .thenReturn(response(error(JhdIbon.QUERY_NOT_FOUND_MESSAGE)));
    ...
    void sanitizedCreateLogContainsAllowlistedEvidenceWithoutSecrets(CapturedOutput output) throws Exception {
        ...
        assertTrue(logs.contains("event=JHD_CREATE"));
    ...
    void recoveredResponseAmountMismatchIsRejected() throws Exception {
        ...
        assertEquals(JhdClient.ErrorType.MALFORMED, error.getType());
        verify(client, times(1)).append(eq(params), any(JSONObject.class));
    }
    ...
    void malformedProviderResponseFailsClosedAfterQueryRecoveryFailure() throws Exception {
        ...
        when(client.queryWithMetadata(params, payOrder.getPayOrderId())).thenThrow(
                new JhdException(JhdClient.ErrorType.AMBIGUOUS, "query timeout"));

        assertThrows(JhdException.class, () -> ibon.executeCreate(params, payOrder, new JSONObject()));
        verify(client, times(1)).append(eq(params), any(JSONObject.class));
        verify(client, times(1)).queryWithMetadata(params, payOrder.getPayOrderId());
    }
    ...
    private static final class TestJhdIbon extends JhdIbon {
        private TestJhdIbon(JhdClient client, JhdMchParamsResolver paramsResolver) {
            super(client, paramsResolver);
        }

        @Override
        protected String getNotifyUrl() {
            return "https://merchant.invalid/api/pay/notify/jhd";
        }
    }
}
