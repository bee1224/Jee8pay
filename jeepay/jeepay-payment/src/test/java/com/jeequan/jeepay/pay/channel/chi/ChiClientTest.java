...
class ChiClientTest {
    ...
    void sendsFormTokenThenBearerJsonCollectAndCachesToken() throws Exception {
        ...
        transport.responses.add(new ChiClient.TransportResponse(200,
                "{\"access_token\":\"synthetic-token\",\".expires\":\"Wed, 12 Aug 2026 03:00:00 GMT\"}"));
        transport.responses.add(new ChiClient.TransportResponse(200, "{\"status\":\"OK\"}"));
        transport.responses.add(new ChiClient.TransportResponse(200, "{\"status\":\"OK\"}"));
        ChiClient client = new ChiClient(transport, Clock.fixed(NOW, ZoneOffset.UTC));
        ...
        assertEquals(ChiClient.TEST_BASE_URL + "Token", token.url);
        ...
        assertEquals(ChiClient.TEST_BASE_URL + "api/Collect", collect.url);
    ...
    void failsClosedForMissingOrUnknownEnvironment() {
        ChiNormalMchParams params = params();
        params.setEnvironment(null);
        ChiClient client = new ChiClient(new FakeTransport(), Clock.fixed(NOW, ZoneOffset.UTC));
        assertThrows(ChiException.class, () -> client.query(params, "P1"));
        assertThrows(ChiException.class, () -> ChiClient.resolveBaseUrl("LOCAL"));
    }

    @Test
    void rejectsMalformedTokenWithoutLeakingCredential() {
        FakeTransport transport = new FakeTransport();
        transport.responses.add(new ChiClient.TransportResponse(200, "{\"error\":\"invalid_grant\"}"));
        ChiClient client = new ChiClient(transport, Clock.fixed(NOW, ZoneOffset.UTC));

        ChiException error = assertThrows(ChiException.class, () -> client.query(params(), "P1"));
    ...
    void classifiesCollectHttpFailureAsAmbiguous() {
        ...
        transport.responses.add(new ChiClient.TransportResponse(200,
                "{\"access_token\":\"synthetic-token\",\".expires\":\"Wed, 12 Aug 2026 03:00:00 GMT\"}"));
        transport.responses.add(new ChiClient.TransportResponse(503, "temporarily unavailable"));
        ChiClient client = new ChiClient(transport, Clock.fixed(NOW, ZoneOffset.UTC));
    ...
    void classifiesCollectHttpFourHundredAsDeterministicBusinessFailureWithAllowlistedFields() {
        ...
        transport.responses.add(new ChiClient.TransportResponse(200,
                "{\"access_token\":\"synthetic-token\",\".expires\":\"Wed, 12 Aug 2026 03:00:00 GMT\"}"));
        transport.responses.add(new ChiClient.TransportResponse(400,
    ...
    void keepsConflictResponseAmbiguousBecauseProviderOrderMayAlreadyExist() {
        ...
        transport.responses.add(new ChiClient.TransportResponse(200,
                "{\"access_token\":\"synthetic-token\",\".expires\":\"Wed, 12 Aug 2026 03:00:00 GMT\"}"));
        transport.responses.add(new ChiClient.TransportResponse(409,
    ...
    private static ChiNormalMchParams params() {
    ...
    private static final class FakeTransport implements ChiClient.Transport {
        private final List<ChiClient.TransportResponse> responses = new ArrayList<>();
        private final List<RecordedRequest> requests = new ArrayList<>();

        @Override
        public ChiClient.TransportResponse post(String url, java.util.Map<String, String> headers,
