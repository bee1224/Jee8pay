...
class JhdMchParamsResolverTest {
    ...
    private JhdMchParamsResolver resolver;

    @BeforeEach
    void setUp() {
        TableInfoHelper.initTableInfo(
                new MapperBuilderAssistant(new Configuration(), "jhd-test"), PayInterfaceConfig.class);
        ...
        resolver = new JhdMchParamsResolver(nativeQueryService);
    }
    ...
    @Test
    void cacheEnabledLoadsJhdParamsFromNativeContext() throws Exception {
        ...
        cached.getNormalMchParamsMap().put(CS.IF_CODE.JHD, validParams());
    ...
    @Test
    void cacheDisabledLoadsJhdParamsFromCanonicalDatabaseRow() throws Exception {
        SysConfigService.IS_USE_CACHE = false;
        when(dbService.getOne(any())).thenReturn(configRow(validJson()));

        JhdNormalMchParams loaded = resolver.resolve(binding("M-TEST", "APP-TEST"));
    ...
    void missingProviderParamsFailClosed() {
        ...
        assertThrows(JhdException.class, () -> resolver.resolve(binding("M-TEST", "APP-TEST")));
    }
    ...
    void malformedProviderParamsFailClosedWithoutCredentialOutput(CapturedOutput output) {
        ...
        assertEquals("JHD 商戶設定格式錯誤", error.getMessage());
    ...
    void incompleteProviderParamsFailClosed() {
        ...
        assertEquals("JHD 商戶設定不完整", error.getMessage());
    }
    ...
    void wrongMerchantBindingCannotReuseAnotherAppsParams() {
        ...
        expected.getNormalMchParamsMap().put(CS.IF_CODE.JHD, validParams());
        when(cacheService.getMchAppConfigContext("M-EXPECTED", "APP-EXPECTED")).thenReturn(expected);

        assertThrows(JhdException.class,
    ...
    private static PayInterfaceConfig configRow(String json) {
        return new PayInterfaceConfig()
                ...
                .setInfoId("APP-TEST")
                .setIfCode(CS.IF_CODE.JHD)
                .setIfParams(json)
                .setState(CS.YES);
    }

    private static JhdNormalMchParams validParams() {
