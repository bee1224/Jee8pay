...
class RyoMchParamsResolverTest {
    ...
    private RyoMchParamsResolver resolver;

    @BeforeEach
    void setUp() {
        TableInfoHelper.initTableInfo(
                new MapperBuilderAssistant(new Configuration(), "ryo-test"), PayInterfaceConfig.class);
        ...
        resolver = new RyoMchParamsResolver(nativeQueryService);
    }
    ...
    @Test
    void cacheEnabledLoadsRyoParamsFromNativeContext() throws Exception {
        ...
        cached.getNormalMchParamsMap().put(CS.IF_CODE.RYO, validParams());
    ...
    @Test
    void cacheDisabledLoadsRyoParamsFromCanonicalDatabaseRow() throws Exception {
        SysConfigService.IS_USE_CACHE = false;
        when(dbService.getOne(any())).thenReturn(configRow(validJson()));

        RyoNormalMchParams loaded = resolver.resolve(binding("M-TEST", "APP-TEST"));
    ...
    void missingProviderParamsFailClosed() {
        ...
        assertThrows(RyoException.class, () -> resolver.resolve(binding("M-TEST", "APP-TEST")));
    }
    ...
    void malformedProviderParamsFailClosedWithoutCredentialOutput(CapturedOutput output) {
        ...
        assertEquals("RYO 商戶設定格式錯誤", error.getMessage());
    ...
    void incompleteProviderParamsFailClosed() {
        ...
        assertEquals("RYO 商戶設定不完整", error.getMessage());
    }
    ...
    void wrongMerchantBindingCannotReuseAnotherAppsParams() {
        ...
        expected.getNormalMchParamsMap().put(CS.IF_CODE.RYO, validParams());
        when(cacheService.getMchAppConfigContext("M-EXPECTED", "APP-EXPECTED")).thenReturn(expected);

        assertThrows(RyoException.class,
    ...
    private static PayInterfaceConfig configRow(String json) {
        return new PayInterfaceConfig()
                ...
                .setInfoId("APP-TEST")
                .setIfCode(CS.IF_CODE.RYO)
                .setIfParams(json)
                .setState(CS.YES);
    }

    private static RyoNormalMchParams validParams() {
