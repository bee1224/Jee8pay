  ...
  >
    ...
    >
      <a-row v-if="vdata.mchType === 1" :gutter="16">
        <a-col span="12">
          <a-form-item label="環境配置" name="sandbox">
        ...
        <a-col span="12">
          <a-form-item label="接口簽名方式(推薦使用RSA2)" name="signType">
        ...
        <a-col span="12">
          <a-form-item label="公鑰憑證" name="useCert">
            <a-radio-group v-model:value="vdata.ifParams.useCert" defaultValue="1">
              <a-radio :value="1">使用憑證（請使用RSA2私鑰）</a-radio>
              <a-radio :value="0">不使用憑證</a-radio>
            </a-radio-group>
          </a-form-item>
        </a-col>
        <a-col span="24">
  ...
  appPublicCert: [
    {
      ...
      validator: (rule, value) => {
        if (vdata.mchType === 1 && vdata.ifParams.useCert === 1 && !vdata.ifParams.appPublicCert) {
          return Promise.reject('請上傳應用公鑰憑證（.crt格式）')
        }
        return Promise.resolve()
      },
    },
  ...
  alipayPublicCert: [
    {
      ...
      validator: (rule, value) => {
        if (
          ...
          return Promise.reject('請上傳支付寶公鑰憑證（.crt格式）')
        }
        return Promise.resolve()
      },
    },
  ...
  alipayRootCert: [
    {
      ...
      validator: (rule, value) => {
        if (vdata.mchType === 1 && vdata.ifParams.useCert === 1 && !vdata.ifParams.alipayRootCert) {
          return Promise.reject('請上傳支付寶根憑證（.crt格式）')
        }
        return Promise.resolve()
      },
    },
